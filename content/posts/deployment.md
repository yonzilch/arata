...
anywhere? `bun preview` does a clean rebuild and then serves `dist/`
...
| `bun build`    | Build once (no cleanup)                        |
| `bun rebuild`  | Clean build (`rm -rf dist` + build) — use this for deploys |
| `bun preview`  | Clean build + serve, to check the prod output locally |
| `bun check`    | `gleam check` + `gleam test`                   |
| `bun clean`    | Remove `dist/` and `build/`                    |
